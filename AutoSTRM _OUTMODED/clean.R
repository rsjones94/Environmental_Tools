...
datasource = 'NFMC_Stitch' # filename of your input data (MUST be .xlsx)
outputname = 'NFMC_Stitch_Clean' # what you want the output to be called - pro or xs will be appended to the end

delimit = '-' # the delimiter you used to separate the name of the shot (e.g., prot2) from its descriptors (e.g., thw)

myr = 'MYR5' # what monitoring year is it? (strings are preferred)
...
for (i in 1:nrow(npro)) {
  ...
  if (currentname != lastname || i == nrow(npro)) { # if there is a change in name or we reach the end of the data
    ...
    nnpro = cbind(nnpro,thw,ws,bkf,tob) # add the four different shot types as columns
